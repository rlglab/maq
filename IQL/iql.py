...
class TrainConfig:
    ...
    offline_iterations: int = int(1e6)  # Number of offline updates
    online_iterations: int = int(1e6)  # Number of online updates
    checkpoints_path: str = f"log/"+env+f"/iql_seed{args.seed}"  # Save path
    load_model: str = ""  # Model load file name, "" doesn't load
...
def convert_trajectories_to_dataset(trajectories):
    """
    將 trajectory 列表轉換為 d4rl.qlearning_dataset 的格式。
    """
    observations = []
    actions = []
    rewards = []
...
# 替換 d4rl.qlearning_dataset(env) 為自定義 load_trajectories
    trajectories = load_trajectories(f"../offline_data/{training_dataset}")
    dataset = convert_trajectories_to_dataset(trajectories)  # 將 trajectory 轉成字典格式
...
@pyrallis.wrap()
def load_IQL_agent(config: TrainConfig, env_id: str, model_path: str):
    config.env = env_id
    eval_env = gym.make(config.env)
...
def load_and_evaluate(config: TrainConfig, env_id: str, model_path: str, n_episodes: int = 100):
    trainer, eval_env, _, _ = load_IQL_agent(env_id=env_id, model_path=model_path)
    ...
    normalized = eval_env.get_normalized_score(eval_score) * 100
    normalized_std = eval_env.get_normalized_score(eval_score_std) * 100
    normalized_max = eval_env.get_normalized_score(eval_score_max) * 100
    print("---------------------------------------")
    print(
        f"Evaluation over {n_episodes} episodes: \n \
        Avg {eval_score:.3f}, Avg D4RL score: {normalized:.3f} \n \
        Std {eval_score_std:.3f}, Std D4RL score: {normalized_std:.3f} \n \
        Max {eval_score_max:.3f}, Max D4RL score: {normalized_max:.3f}"
    )
    print("---------------------------------------")

    return eval_score, normalized

def load_IQL_and_decide_actions(trainer, state, state_mean, state_std):
    state = state.copy()
    state = normalize_states(state, state_mean, state_std)
    return trainer.actor.act(state, device=trainer.device)


if __name__ == "__main__":
    # set_seed(args.seed)
    train()
    # load_and_evaluate(env_id="door-human-v1", model_path="log/door-human/iql/checkpoint_1999999.pt", n_episodes=100)
